                ...
                with pm.frameLayout(l="Set/Remove Defaults", bs="out", mw=2, mh=2, cll=True, cl=True) as setFrame:
                    with pm.columnLayout(rs=2, adj=True):
                        ...
                        pm.button(
                            ...
                            bgc=self.colSet,
                            ann="Set defaults on the selected objects using the selected channel box attributes",
                        )
                        pm.button(
                            l="Remove Defaults",
                ...
                with pm.frameLayout(l="Reset", bs="out", mw=2, mh=2) as resetFrame:
                    with pm.formLayout(nd=100) as resetForm:
                        b6 = pm.button(
                            ...
                            bgc=self.colReset,
                            ann="Reset the selected objects. Uses transform standards if no defaults are defined for translate, rotate, and scale",
                        )
                        b7 = pm.button(
                            l="Default",
                            c=pm.Callback(reset),
                            bgc=self.colReset,
                            ann="Reset the selected objects using only stored defaults, if any",
                        )
                        b8 = pm.button(
                            l="Transform",
                            c=pm.Callback(resetTransform),
                            bgc=self.colReset,
                            ann="Reset the selected objects using only transform standards for translate, rotate, scale (eg. 0, 0, 1)",
                        )
                        b9 = pm.button(
                            l="All",
...
def getDefaults(node):
    ...
    if dattr is not None:
        ...
        for k, v in defaultsRaw.items():
            ...
            if k == DEFAULTS_ATTR:
                pm.warning(
                    "skipping attribute {0}. it stores defaults and is therefore unable to have a default".format(k)
                )
                continue
